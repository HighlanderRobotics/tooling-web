...
export const _questions: ServerQuestion<string | string[]>[] = [
	...
	{
		id: 'requierd-attire',
		readableName: 'What attire is required for the shop?',
		type: QuestionType.MULTI_SELECT,
		answers: {
			'long-pants': 'Long pants',
			'long-hair-tied': 'Long hair tied back',
			'close-toed-shoes': 'Close Toed Shoes',
			'baggy-sweaters': 'Baggy sweaters',
			'short-sleeves': 'Short sleeves or sleeves rolled above the elbow',
			'safety-glasses': 'Safety Glasses',
			'reading-glasses': 'Reading Glasses with protective safety glasses'
		},
		correctAnswer: ['long-pants', 'long-hair-tied', 'short-sleeves', 'safety-glasses']
	},
	{
		id: 'when-finished',
	...
	},
	{
		...
		answers: {
			...
			support: 'Ask a mentor to support you.'
		},
	...
	},
	{
		...
		answers: {
			...
			notify: 'Notify the parent or mentor supervising.',
	...
	},
	{
		...
		correctAnswer: ['avoid-mistakes', 'aware-safety-glasses']
	},
	{
		id: 'gloves',
		readableName: "When shouldn't you wear gloves?",
		type: QuestionType.SINGLE_SELECT,
		answers: {
			welding: 'While welding',
			hot: 'When handling something hot.',
			'powered-tool': 'When using any powered tool.',
			sharps: 'When working with sharp objects or splintered wood.'
		},
		correctAnswer: 'powered-tool'
	},
	{
		id: 'defective-tools',
...
export const actions = {
	default: async (event) => {
		...
		if (percent >= 80) {
			...
			throw redirect(
				303,
				`/tools/lab-certification/quizzes/safety-quiz/pass?score=${encodeURIComponent(
					score
			...
			throw redirect(
				303,
				`/tools/lab-certification/quizzes/safety-quiz/fail?score=${encodeURIComponent(
					score
				)}&total=${encodeURIComponent(total)}&incorrect=${encodeURIComponent(
					JSON.stringify(incorrectQuestions)
				)}`
