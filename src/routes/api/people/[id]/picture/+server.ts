...
export const GET: RequestHandler = async ({ params, url }) => {
    ...
    if (person.image) {
        ...
        const response = await fetch(person.image.replace(/=s\d+-c$/, `=s${size}-c`));
        ...
        const backgroundColor = backgroundColors[id.split('').reduce((a, b) => a + b.charCodeAt(0), 0) % backgroundColors.length];
        ...
        context.fillText(person.name.split(' ').map(a => a[0]).join(''), size / 2, (size / 2) + (size / 45));
